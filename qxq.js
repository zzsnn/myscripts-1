...
软件名：趣星球
【REWRITE】
匹配链接（正则表达式） https://api.xqustar.com/api/user/loginByPassword
对应重写目标   https://raw.fastgit.org/byxiaopeng/myscripts/main/qxq.js
食用方法：重新登录趣星球即可获取
10 8-18/2 * * * https://raw.fastgit.org/byxiaopeng/myscripts/main/qxq.js
*/
    ...
    return new Promise((resolve) => {
        ...
        $.get(url, async (err, resp, data) => {
            try {
                result = JSON.parse(data)
                if (result.code == 200) {
                    $.log(`【欢迎吊毛用户】：${result.data.inviteCode}\n`)
                    $.log(`【你的邀请码】：${result.data.nickname}\n`)
                    $.message += `【欢迎吊毛用户】：${result.data.inviteCode}\n`
                    $.message += `【你的邀请码】：${result.data.nickname}\n`
                    await $.wait(2000)
                    await signin()
                } else {
    ...
    return new Promise((resolve) => {
        ...
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if (result.code == 200) {
                    $.log(`【幸运大转盘】 剩余次数：` + num)
                    $.log(`【恭喜获得】：${result.data.number}星钻\n`)
                } else {
